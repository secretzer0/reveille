...
def doctrine_block(name, agent_type, version=__version__):
    ...
    return (
        ...
        f"has been taken from you and nothing will be until it joins. Write your\n"
        f"handover NOW, while you are the one holding the context:\n"
        f"`memory_add(kind=\"state\", ...)` -- task, branch, next step, open threads,\n"
        f"what is uncommitted and where. The new body reads it first. Then carry on:\n"
        f"if it never arrives, nothing about your situation changed.\n\n"
