...
def doctrine_body(name, agent_type):
    ...
    return (
        ...
        f"server-side.\n\n"
        f"Startup: `join()`, then `rehydrate()` PAGE 1, then arm. Page 1 row 1 is\n"
        ...
        f"writes one; a NEW body calls `digest(mentor=\"$REVEILLE_MENTOR\")` first,\n"
        ...
        f"is per BODY-SESSION and the boundary is invisible from the far side of\n"
        f"it. \"ALREADY ARMED\" IS A CLAIM ABOUT A LIVE PROCESS, VERIFIED BY\n"
        f"LIVENESS, NEVER BY MEMORY -- at boot arm unconditionally; a duplicate\n"
        f"watcher costs one duplicate ring, a skipped one costs every ring.\n\n"
        ...
        f"so rm THAT, never a glob. An entry left behind is re-printed by the next\n"
        f"watcher process, which is an acked ring waking you on every re-arm.\n"
