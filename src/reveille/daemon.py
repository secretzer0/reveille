...
CHANGES_ENTRIES = (
    ...
    ("0.2.313",
     '0.2.313 TWO FIELD FINDINGS FROM THE FIRST LIVE BOOT RING, both in code shipped\nthe same evening.\n\nA RESUME IS NOT A BODY WITHOUT MEMORY. The session watcher keyed arrivals on\nPID, and `claude --resume` starts a new process carrying the SAME\nconversation. Measured: an interrupt-and-continue replaced pid 1691395 with\n1700106 five seconds apart, both sessionId 4a8f2471, and the watcher rang\nreason=boot -- asking a body that had rehydrated two minutes earlier to read\nall 44k characters of its digest again, on the same day the operator asked for\nno token to be spent on a poll with nothing to return. The descriptor already\nsaid which conversation it carried. The watcher now remembers conversations it\nhas seen (bounded at 256, oldest evicted) and rings boot only for one it has\nnot; the first census seeds that memory, so a resume after a waked restart is\nrecognised too. A descriptor that cannot name its conversation still rings --\na body we cannot prove has memory is told where its memory is, because that\nfall costs a turn and the other costs a mind.\n\nA COUNT THAT CHANGED MEANING KEPT ITS NAME. `items` held rows AND messages\nuntil the index cut over, then MESSAGES ONLY, and `rows = len(items)` went on\nbeing called rows. The first healthy fold after 0.2.309 printed `input: 4 rows`\nover a 453-row index and raised `[WINDOW: indexed 453 rows but the store only\noffered 4]` -- the window check built to catch the 0.2.305 truncation, firing a\nfalse alarm on a good note. A false alarm on every fold is how an alarm stops\nbeing read. rows is len(rows_kept) now and messages has its own name; the\nheader prints both. Same class as 0.2.305, one field over: a cutover changed\nwhat a thing meant and every reader kept the old meaning.'),
    ("0.2.312",
     '0.2.312 GATES LEAK WHAT THEY SPAWN -- THE SECOND TIME, IN THE SAME FILE.\n\n63 reveille-waked daemons were running on this machine, the oldest 5h56m, one\nper suite run, accumulating all day while I ran the suite sixty times.\n\nTHE LEAK GATE ALREADY EXISTED AND WAS SCOPED TOO NARROWLY. 0.2.293 added\ntest_driving_the_hook_leaks_no_daemon after twelve leaked under the fixture\nname `ana`, and it reaps `ana` -- by name. The gate that drives the hook as\n`nobody-is-reachable-here` therefore walked straight past it: the hook finds no\ndoorbell, spawns a waked exactly as it is supposed to, subprocess.run returns,\nand the daemon outlives the test. A fix aimed at the instance rather than the\nclass catches nothing the second time.\n\nAn autouse fixture reaps every reveille-waked that appears during ANY test in\nthat file, by pid diff. A per-test reaper cannot be forgotten by the next test,\nwhich a per-test assertion demonstrably can. Verified: a full suite run now\nleaves zero.\n\nKilled by PID from a ps census, never `pkill -f` -- the pattern matches the\nshell asking, which is the lesson that cost an exit 144 earlier in this same\nrelease.'),
    ("0.2.311",
     '0.2.311 THE DAEMON SEES THE BODY COME AND GO (operator, 2026-09-20: "on boot,\nif the reveille mcp is enabled, it should rehydrate -- this can be handled by\nthe waked noticing a new cli having joined so its hive-mind memory comes back\nwithout any special commands. And when the waked notices the agent exits, the\nwaked sends a command to the server to reconcile its hive-mind so it is fully\nup-to-date when it returns").\n\nTWO EDGES ONLY THIS DAEMON CAN SEE, and it was already looking at them: waked\nreads the CLI session descriptors to ring the doorbell, so a census of the same\nfiles costs nothing new.\n\nARRIVAL RINGS reason=boot. "rehydrate at boot" has been doctrine since the\ndigest shipped and NOTHING EVER CARRIED IT -- the instruction existed, the\nevent did not, and a fresh body only learned its hive had a memory if something\nelse happened to wake it. The ring costs the one turn that body was going to\nspend booting anyway, and the doctrine now names the reason so it can be acted\non: page 1 row 1 is the digest, and that is the whole boot read.\n\nTHE LAST DEPARTURE RECONCILES. POST /agent/digest when the final session in an\nagent\'s directory goes, so what that body did is folded while it is away and\nthe next one rehydrates something current rather than an hour stale. A\ndeparture that leaves other sessions alive is NOT an exit -- the identity is\nstill working in another window, and a fold there would only be superseded by\nthe next one.\n\nTHE BROKER OWNS EVERY REASON TO SAY NO: an interval guard, a one-fold-at-a-time\nlock fleet-wide, an activity check. So the request is fire-and-forget and a\nrefusal is an answer, not an error -- none of that logic belongs in a daemon\nthat cannot see the other ten identities.\n\nAND THE FIRST CENSUS RINGS NOBODY. waked restarts on every deploy, so counting\nthe sessions it finds at startup as ARRIVALS would wake every live body on the\nmachine to fetch memory it already has -- eleven turns, every deploy. First\nlook seeds the set in silence.'),
    ("0.2.310",
    ...
    ("0.2.307",
     '0.2.307 THE INDEX REBUILDS, SO IT MUST SEE EVERYTHING (field regression,\nshipped in 0.2.305 and measured on the live broker within ten minutes of the\ndeploy that carried it).\n\nA later fold emitted `input: 1 rows` with RULES, DECISIONS and LESSONS all\n`(none)` -- a 732-character note where the prior carried 445 rows. Not a\nrefusal, not an error, and every gate green: a well-formed digest of one hour\nof the hive, presented as the whole mind.\n\ndigest_inputs windows the rows by `since = prior.created_ns`, which was RIGHT\nfor a merging fold. That fold ACCUMULATED -- each step folded the batch into\nthe prior note -- so handing it only what arrived since that note was exactly\ncorrect and nothing else would have been. The INDEX rebuilds the note from\nrows_kept every single time, so the same window silently became "forget\neverything older than an hour". The rows are no longer windowed; `since` keeps\nwindowing the MESSAGES, which is the only thing it was ever for.\n\nTHIS IS THE SECOND TIME THIS EXACT SHAPE HAS SHIPPED TODAY. In 0.2.298 the\noutput side lost its accumulation -- the carried fold kept the prior by\nre-transcribing it, a stateless step did not, and `running = ""` meant every\nfold after the first superseded the whole note with one hour of traffic. The\noperator found it by asking whether a second fold would be incremental. The fix\nthen was prior_text seeding. The index deleted the need for that seeding and\nleft the INPUT window standing, which produces the identical symptom one layer\nover.\n\nTHE LESSON, because twice is a pattern: WHEN THE OUTPUT SIDE STOPS\nACCUMULATING, EVERY INPUT WINDOW THAT ASSUMED IT IS NOW A TRUNCATION. An\nincremental input and a rebuilding output cannot both be right, and nothing in\neither half knows about the other -- the window is in the store, the rebuild is\nin the daemon, and each is locally correct.'),
    ("0.2.306",
    ...
    ("0.2.303",
     '0.2.303 A NOTE PER AGENT, NOT PER ROOM (step 3 of the march).\n\nFIRST, A FIGURE I QUOTED THAT NO AGENT EVER SEES. "1360 rows, 92% of the\nceiling, two weeks of headroom" was the ALL-LIVE count; room scope already\nseparates the fleet, and the real per-body views are:\n\n    Reveille2.0   445 rows   30% of ceiling    8 rows/day   18.3 weeks\n    OverSiteAI    922 rows   62% of ceiling   32 rows/day    2.4 weeks\n\nSo the pressure is not the hive, it is the BIG ROOM, where 17 agents share 915\nrows and every one of them carries all of it.\n\nTHE SIGNAL WAS ALREADY IN THE SCHEMA. The fleet names bodies after what they\nown and the entity extractor pulls the same words out of the rows -- measured\non those 915 rows, the top of the entity vocabulary IS the roster: roc-api 175,\nshared 133, controller-api 102, mobile 100, roc-ui 85, minimal-mobile 74,\ndeployment 37, streaming 16, vendor-api 15, controller-ui 12. A correspondence\nthat good is a scoping signal sitting in the database, and it costs no model\nand no embedding.\n\nFOUR TIERS, deterministic, newest-first inside each:\n  0  IT BINDS -- doctrine, contract, global. A peer breaks these by not knowing\n     them, so they are never what gets dropped.\n  1  I WROTE IT.\n  2  IT NAMES MY COMPONENT, by the entities already stored.\n  3  EVERYTHING ELSE.\n\nMeasured across the OverSiteAI roster, the first three tiers come to 158-417\nrows against 922 unscoped -- 10-28% of the ceiling instead of 62% -- and\nroc-api-dev\'s headroom goes from 2.4 weeks to about 35.\n\nIT IS AN ORDER, NOT A FILTER. Every tier still fits today; the ranking decides\nwhat survives the day the store outgrows the ceiling, which is the only day the\ndistinction matters. Driven at budget 200 on the real corpus, every agent keeps\nall 151 binding rows, authored rows fill the rest, and a body that has written\nlittle falls through to recency rather than to nothing.\n\nAND A GENERIC NAME PART CANNOT OVER-MATCH. roc-api-dev claims the tokens\n{roc-api, roc, api}, and entities compare WHOLE -- so `api` matches an entity\nliterally called `api` and never `vendor-api`. Of the 175 rows it matched, the\nentity that did it was `roc-api` every time.\n\nSTILL OPEN, and it grows: the binding tier took 151 of a 200-row squeeze. As\ndoctrine and contracts accumulate they will crowd out every other tier, and the\nanswer then is a rule for which binding rules bind THIS agent -- not a bigger\nnumber.'),
    ("0.2.302",
     '0.2.302 A TABLE OF CONTENTS, NOT A PHOTOCOPY (steps 1 and 2 of the march the\noperator set: reach contradiction detection, testing each step, course\ncorrecting).\n\nTHE CITATION LOST ITS DATE. `[lesson:9d384223 2026-09-16]` costs 22.94 tokens a\nrow and `[lesson:9d384223]` costs 11.94 -- 19175 tokens across the live store,\nfor a field created_ns already holds and every read already returns. A\ncitation\'s whole job is to RESOLVE; anything in it the lookup returns anyway is\nrent. EIGHT HEX STAYS: 4 hex already collides 8 times in today\'s 2169 rows and\nprojects to 1636 collisions at a year of growth, 6 hex to 6.4, 8 hex to 0.02.\nThe cheap citation was worth 19175 tokens; a short id is worth 2 a row and a\nwrong row. 19 tests carried the old shape and were cut over with it -- a\ngrammar change has exactly that blast radius, and existing digests are a CACHE,\nregenerable from the store, which is the whole reason the store decides truth.\n\nAND THE NOTE STOPPED BEING WRITTEN. digest_index() builds RULES, DECISIONS and\nLESSONS from the store alone: each row\'s own first sentence, truncated, ending\nin its tag. No model, no batches, no step that can decline a row, no invented\ntag to license, no retry, no 22-batch first fold.\n\n    written fold   431 of 1360 rows   49996 tok   ~20 GPU-min   57% coverage\n    index         1360 of 1360 rows   45848 tok      32 ms     100% coverage\n\nFidelity goes UP. A written restatement is a paraphrase that can drift from the\nrow it cites; a truncation IS the row\'s words, and the exact text is one\nrecall() away for any line a reader wants whole. What falls is prose quality,\nwhich was never the thing being stored. COVERAGE STOPS BEING A MEASUREMENT AND\nBECOMES A PROPERTY: every selected row appears, because appearing is what the\nfunction does.\n\nTWO COURSE CORRECTIONS, both from measuring the output rather than trusting the\ndesign. Splitting the first sentence on ":" as well as ".!?" produced 47 stubs\nlike "S3 review rulings:" -- a colon in this corpus INTRODUCES the content\nrather than ending a thought. And one title length for every kind starved the\nrules while spending characters where a lesson\'s SLUG had already said it:\nlesson@30 with other@70 gives 7 stubs at 45848 tokens, against 17 stubs at\n43596 for a flat 45.\n\nTHE ROW BUDGET MOVED WITH THE LINE FORMAT. DIGEST_INDEX_ROW_TOKENS was 116 when\na model wrote each line and is 34 now, so the same 50000 ceiling carries 1470\nrows rather than 431 -- the whole live store, with nothing left out. A derived\nnumber that outlives what it was derived from is how a budget starts lying.\n\nWHAT THIS DOES NOT FIX, and the growth curve says so plainly: the store adds\n36-150 rows a day and the index is at 91% of the ceiling. Two weeks. Per-agent\nscoping is the next step and it is the one that divides the problem instead of\nshaving a constant off it.'),
    ("0.2.301",
     '0.2.301 MERGING CANNOT BOUND IT; SELECTION CAN.\n\nTHE HYPOTHESIS WAS FALSIFIED BEFORE IT COST A DEPENDENCY. Compaction shipped in\n0.2.300 on the assumption that a rolling digest accumulates duplicates. Three\nindependent measurements over the same 1360 live rows say it does not:\n\n  word-Jaccard, exact all-pairs within kind   4 near-duplicate pairs (0.5s CPU)\n  TF-IDF, word bigrams + char 5-grams         the same 4; ZERO at cosine >= 0.40\n  the writer itself, 15 windows, 934 GPU-s    7 lines of 258, 1% of tokens\n\nThe rows genuinely differ. A stronger similarity signal did not find more,\nwhich is the evidence that a still stronger one -- an embedding model -- would\nnot either: the best real pair in the store (one ruling restated by two agents)\nscores 0.35, and that IS the ceiling of duplication here. So NO embedding\nmodel, NO vector database. At 1360 rows a vector set is ~2 MB and exact cosine\nis one matmul; FAISS and every ANN index exist to trade recall for speed at a\nscale three orders of magnitude away. If the store ever reaches ~500k rows,\nsqlite-vec keeps the vectors in the file that already exists.\n\nWHAT WAS ACTUALLY WRONG IS THAT NOTHING BOUNDED WHAT A FOLD WAS OFFERED. A\nfirst run pulled every live row the agent could read: 1360 rows, ~910k chars,\n59 batches, and the note stopped wherever the batches ran out. That was\nreported as 57% COVERAGE -- a number measuring the size of the STORE, not the\nquality of the fold. 0.2.278 said rows were "the small, load-bearing part" and\nwindowed only messages; at 1360 rows that is no longer true.\n\ndigest_select() bounds it, deterministically, BEFORE any GPU runs. Every\nbinding rule first -- doctrine and contract, what a peer breaks by not knowing\nit -- then decisions and lessons newest-first with what is left, and time order\nrestored so the note still reads chronologically. The budget is not a second\nnumber kept in step with the ceiling by hand: it is the ceiling CONVERTED,\nonce, through a MEASURED cost of 116 tokens per stored row (29951 tokens over\n258 folded rows). 50000/116 = 431 rows.\n\nMEASURED ON THE LIVE STORE: 1360 rows -> 431 offered, 929 left out. 59 batches\n-> 22, so the first-fold penalty falls 63%. Kept: all 57 doctrine, all 189\ncontracts, then 141 lessons and 44 decisions back to 2026-07-28.\n\nA ROW LEFT OUT IS NOT LOST. It is live, queryable, one recall() away -- the\nnote is a CACHE of the hive and never the hive, and the header says so:\n`[bounded: 929 older row(s) not carried -- live in the store, reachable with\nrecall()]`. The note must never imply it is the store.\n\nAND COVERAGE FINALLY MEANS SOMETHING. A bounded fold offers what it INTENDS to\ncarry, so the number measures the fold rather than the store.\n\nNAMED BECAUSE IT IS NOT SOLVED: the binding kinds take 246 of 431 and they only\ngrow. Contracts alone will eventually fill the budget, and when they do the\npolicy needs a rule for which binding rules bind THIS agent -- not a bigger\nnumber.'),
    ("0.2.300",
    ...
    ("0.2.292",
     "0.2.292 THE GATE TESTS REACHABILITY, NOT A PROCESS (operator direction:\nleave only the systemd-backed waked, stop arming watchers). The Stop hook\ndecided by `pgrep wake-watch <role>`, and its own comment already admitted\nwhat that costs: a process exists is not the harness will be notified, which\nis exactly how an architect armed a watcher with `cmd &`, satisfied the\npgrep with an orphan writing to nothing, and went deaf with every control\ngreen (2026-08-19).\n\nSince 0.2.290 a ring is not only a file, so the hook can ask the question it\nactually cares about. doorbell.reachable() answers it in TWO HALVES, both\nrequired because each alone is a different deafness: a waked must hold this\nidentity's spool lock (or no ring is ever written), and the identity's own\ndirectory must hold a live interactive session with the reveille MCP (or a\nwritten ring never becomes a turn). The second half reuses the DELIVERY's\nchecks exactly, so the gate cannot pass on a path the ring itself refuses.\nA body the doorbell reaches needs no watcher; every other body is blocked\nexactly as before, and REVEILLE_DOORBELL=off puts everyone back on one.\n\nIT FAILS CLOSED, which is the only reason it is safe in a hook: a refusal,\na crash, a missing interpreter or an unimportable module all leave the stop\nBLOCKED. Never `|| exit 0`. Gated by driving the real hook with a dead\ninterpreter and asserting the block.\n\nAND /agent/digest IS GUARDED AT LAST. Every web API route goes through\n_guard, which turns the store's auth vocabulary into status codes -- except\nthis one, registered bare since it was added. A token bound to another\nidentity produced an uncaught store.AuthError, a full ASGI traceback in the\nbroker log and a 500 to a hook that only ever wanted a verdict. The hook\nfires after EVERY turn, so one mismatched body sprays the log. Now a 401.\n\nHOW BOTH WERE FOUND IS THE PART WORTH KEEPING: a new gate drove the real\nhook with dict(os.environ), so it carried this machine's LIVE credential and\nPOSTed production under the fixture name `ana`. The broker's own log said\n`ana wake rejected: name_mismatch (bound to native-reveille-devops)`. The\ntest is now sealed -- every REVEILLE_* stripped by name -- and the seal is\nitself gated, because a credential leaking back in is a regression only\nproduction would notice.\n\nagent-stop-hook is a BAKED INPUT, so reveille-agent moves 0.2.43 -> 0.2.44\nacross all four pin sites."),
    ("0.2.291",
     "0.2.291 THE DOORBELL RINGS ON WINDOWS TOO -- BEST EFFORT, UNVERIFIED\n(operator: build it now if it is low effort). 0.2.290 refused on any\nplatform without socket.AF_UNIX and called that the end of the story. It\nwas not, and the correction matters more than the code: I had searched the\nCLI bundle for a Windows branch, found nothing, and offered THAT as evidence\nof absence. The pattern was broken, not the bundle. A validated fixed-string\nsearch -- proven against a known-good sample first -- finds the CLI's own\nflag help:\n\n    --messaging-socket-path <path>   Cross-session messaging server path: a\n    Unix domain socket on Mac/Linux, a \\\\.\\pipe\\ name on Windows\n\nSo Windows has the SAME cross-session messaging and only the transport\ndiffers. The branch is therefore a property of the PATH, not of sys.platform\n(transport_for): a \\\\.\\pipe\\ name is opened as an ordinary file and needs no\nAF_UNIX, anything else is a unix socket, and a machine that somehow served\nboth would be served correctly. macOS needed nothing then and needs nothing\nnow -- it is Linux with its sockets under /tmp.\n\nON A THREAD, AND NOT AS A REFINEMENT: file I/O has no timeout, the doorbell\nruns INLINE in write_ring, and a pipe nobody drains would hang the daemon's\nwhole ring path -- the one thing this feature promised never to touch. The\nwrite runs on a daemon thread joined with the connect timeout; a stuck one\ncosts a file handle, not a ring. Gated by mutation: run it inline and the\ngate stalls for the full sleep instead of returning in 0.3 s.\n\nAND _alive's POSIX GUARD IS NOW LOAD-BEARING. On Windows signal 0 IS\nsignal.CTRL_C_EVENT, so os.kill(pid, 0) delivers a console interrupt rather\nthan probing; while the doorbell refused on Windows that guard was\nbelt-and-braces, and now it is the only thing between a liveness check and\nan interrupt aimed at a working session. The price is that a dead CLI's\ndescriptor reads as live there and we ring a pipe nobody serves -- a refused\nopen, cheap and logged.\n\nUNVERIFIED, SAID PLAINLY AND IN THE CODE: no Windows body exists here to run\nit against. The pipe writer is gated against a FIFO, which proves it emits\nthe protocol over file I/O and proves nothing about Windows."),
    ("0.2.290",
     "0.2.290 A RING CAN RING (operator direction 2026-09-20, measured the same\nhour). Until now a ring was a FILE and nothing more: waked spooled it, and\nthe body had to be holding a `wake-watch` process whose EXIT the harness\nwould notice. That shell had to be re-armed every turn, died with the\nsession, forked into one-shot vs `--follow`, and went silently deaf whenever\nthe arming was lost -- an entire defect class descended from the fact that\nnothing could START a turn from outside.\n\nSomething can. A Claude Code CLI publishes an inbox on a unix socket and\nprints the protocol itself at startup: an auth line then a user line,\nnewline-delimited JSON. MEASURED on 2.1.272, two bodies on one machine: the\ntarget went idle -> busy 11 s after one line, ran a command, wrote a file,\nreplied, and went idle again, with nothing typed into it.\n\nwaked now rings that doorbell, and FOUR things are checked before a byte is\nsent, because a ring is a DELIVERY and a misrouted one hands an agent's mail\nsubject to another body. A DIRECTORY NAME IS NOT AN IDENTITY (operator): the\ndirectory comes from the registry (24286) and never from the name, the\ndirectory must still CLAIM this identity in its own settings.local.json\n(ruling 24332 s1's check, applied to delivery), the session must be a LIVE\nINTERACTIVE CLI -- a descriptor is a claim about a process, and a `-p` run\nhas no prompt to wake -- and the directory must have the reveille MCP, or\nthe line would spend a turn telling a body to call inbox() it does not\nhave. Every live session in that directory is rung, ALL of them, because a\nduplicate ring costs one turn and a skipped one costs every ring.\n\nTHE SPOOL IS STILL THE MAILBOX. The socket reaches a RUNNING session and\nnothing else, so moving the durable queue there would trade deafness for\nloss. write_ring() files the ring FIRST, every time, and rings afterwards,\nand the whole doorbell is wrapped: refuse, hang, raise or vanish, the ring\nis already on disk and write_ring still returns its path. That is the gate\nthe change rests on, and its mutation -- unwrap the knock -- reds.\n\nPORTABILITY IS CHECKED, NOT ASSUMED. The socket PATH is never ours to build\n-- we read `messagingSocketPath` out of the descriptor -- so the CLI's own\nlayout differences cost nothing: macOS under /tmp or /private/tmp (with its\nfallback from XDG_RUNTIME_DIR when the path would pass the 103-byte sun_path\nlimit), Linux under /run/user/<uid>/cc-socks, Termux its own prefix. The\nTRANSPORT is what varies: CPython exposes no socket.AF_UNIX on Windows, so\nthere the doorbell refuses BY NAME and wake-watch is the whole delivery. And\nliveness never calls os.kill off posix, because on Windows signal 0 IS\nsignal.CTRL_C_EVENT -- the POSIX probe idiom becomes a console interrupt\naimed at the session it was asking about.\n\nADDITIVE, NOT A CUTOVER, and deliberately so: wake-watch is untouched and\nstill the delivery of record. This is one release of field evidence away\nfrom being allowed to replace it, and the protocol is the CLI's own\ninternal surface with no compatibility promise. REVEILLE_DOORBELL=off is\none env line and a restart. Native shape only -- the container shape is\nunmeasured and nothing here assumes it."),
    ("0.2.289",
    ...
    ("0.2.283",
     "0.2.283 ONE WAKED PER HOST (operator 24206: \"a SINGLE waked could do the work\nfor ALL local agents\"; ruled 24208/24213/24286). Measured first: ELEVEN\nreveille-waked processes on one workstation, one per agent, each with its\nown converge and its own \"which one is mine\". waked is plumbing -- it holds\na socket and turns rings into spool files, and nothing downstream cares\nwhich process wrote the file -- so one process can hold N sockets and feed\nN spools. The container shape is this same code with N=1 and never notices.\n\nTHE REGISTRY IS A PATH, NEVER A SECRET. A credential lives only in its own\ndirectory, and nothing mapped a NAME to that directory, so a host-wide waked\ncould not enumerate what it serves. `reveille init` -- the one writer of a\ncredential -- now also writes `~/.reveille/agents/<name>` holding that\nabsolute path, 0600, atomic, last init wins. The token is read from THAT\ndirectory at attach time, so rotation and body swaps need no registry write,\nand a stale entry (no directory, or a directory that now names somebody\nelse) is SKIPPED WITH ITS REASON and never deleted -- pruning is an operator\nact.\n\nOWNERSHIP IS THE LOCK IT ALWAYS WAS: `reveille-waked --host` is a singleton\non ~/.reveille/host.lock, and then takes EACH agent's existing spool flock\nbefore opening that identity's socket. So a per-agent waked and a host waked\ncontend on the same lock as ever, the loser leaves that identity alone, and\nthe Stop hook's liveness probe (\"does somebody hold MY spool lock\") is true\nfor either shape without knowing which runs. Re-enumeration is one opendir\nevery 30 s and immediately on SIGHUP, so an identity added while it runs\nattaches without a restart. Per-identity state that used to come from the\nprocess environment and the cwd -- token, credential file, parked secret,\nwedge artifact -- is now addressed per run, so N identities share a process\nwithout sharing any of it.\n"),
    ("0.2.282",
     "0.2.282 WHEN THE PARENT LEAVES, THE WATCHER LEAVES (operator 24200, architect\n24202). wake-watch is the one-shot a body arms each turn, and its owner IS\nthe shell that armed it -- yet a harness that stopped tracking that shell,\nor a session that ended, left the python behind, reparented to init and\nwaiting on a spool nobody would read. Seven of those were counted on one\nhost in one evening. Now the kernel names the owner: PR_SET_PDEATHSIG on\nLinux kills the watcher the instant its parent exits, and the portable half\nis a ppid check on every wait tick, which shrank from 30 s to 2 s to carry\nit (inotify still wakes it on a ring at once). No GUID, no file, no\npattern-kill: the mechanism is the bookkeeping, and every live wake-watch\nfor a role now belongs to a live session by construction. reveille-waked is\ndeliberately untouched -- it is the identity's daemon, a flock singleton\nthat outlives every session and carries the identity across a swap.\n"),
    ("0.2.281",
...
"""),
    ...
    ("0.2.267",
     "0.2.267 THE GATES THAT NEVER RAN. Twenty-two files under tests/ wear\n_gate or _smoke in their name. They drive a real broker over a real client -- the only\ncoverage of the wire an in-process suite cannot give -- and every one of them\nwas a Makefile target and NOTHING ELSE. ci.yml runs `uv run pytest tests/ -q`;\n`pytest --collect-only` matched none of them. They had never run in CI once.\n\nWHAT THAT COST, measured the first time they were run. THREE were red on main\nbefore any of this began, each one release's drift sitting in a file nobody\nexecuted: joinhere_smoke asserted the hook's OLD name while install.py writes\n`reveille-stop-hook`; readmit_gate expected a bare join() to undo a deliberate\nleave(), which leave-stickiness ended -- the daemon says `rooms=0 skipped=1`\nout loud and only join(room=...) clears it; smoke_ws read one frame after\nconnecting and called it the wake, which the unconditional attach greeting\n(0.2.253) made wrong, and its anti-storm check treated ANY frame as a ring for\nthe same reason. A gate nobody runs is documentation with an exit code.\n\nTHEY NEEDED NOTHING A RUNNER LACKS, which is why they did not have to wait for\nchromium: scratch.py is `subprocess.Popen([\"reveille-daemon\"])` on a free port.\nThe wiring is a parametrized subprocess test, the repo's own shape from\ntest_the_bank_travels_between_installs.py. 12 wired, 10 named in NOT_WIRED with\nthe measured reason each cannot run yet -- and THAT LIST HAS ITS OWN GUARD,\nbecause a file nobody runs is the defect this exists for: a new gate lands in\none list or the other or the suite goes red. It earned itself twice on its\nfirst runs, finding the THIRTEEN files nobody had counted (we had all said\n\"the eight client gates\"), and that smoke_ws.py wears the word at the FRONT,\nso a `*_smoke.py` glob misses it.\n\nONE STUB, NAMED: cmd_join_here shells out to `claude mcp remove` and `add`,\nand a hosted runner has no claude. The stub RECORDS ITS ARGV and the gate\nasserts it -- a stub that only exits 0 is a bypass; one that records is a gate\non the launcher's side effect. The property lives in the argv: the\nregistration carries ${REVEILLE_TOKEN} as a TEMPLATE, never a value.\n\nsmoke_ws's human half now LOGS IN. It posted with an AGENT's bearer token and\nfrom:\"operator\", which worked while the plane was inferred from that string;\nsince unbound tokens went read-only an unbound token on /send answers 401, so\na human IS a session principal and that half had quietly become an agent's\nparentless broadcast -- ringing nobody, while the test asserted the opposite.\nIts `from` now expects the AUTHENTICATED user rather than the string the\nclient typed, which is 14056 and the stronger claim.\n\nTWO SCHEDULING TRUTHS FELL OUT. The wired gates share one xdist group\n(`--dist loadgroup`) so they serialise against each other; a dozen brokers\nracing on the same cores is a timing test nobody wrote. And moving tests\nbetween workers exposed that `daemon._tts_q` is a MODULE GLOBAL drained to a\nsentinel, so an item one test leaves behind is spoken by the next --\ntest_script_writer now drains it, as its own fixture already did.\nsweep_scheduler_smoke stays OUT: green alone, red on the runs that took 85 s\ninstead of 56. Driving its clock is its own commit; widening its window would\nbe gaming it (13760).\n"),
    ("0.2.266",
     "0.2.266 THE RUNTIME NEEDS ITS OWN HTTP CLIENT (hotfix: 0.2.265 crashlooped\nin production, `Restarting (1)`, and autodeploy HELD itself as designed).\n\nWHAT BROKE. authlib does a bare `import httpx` inside\nintegrations/httpx_client, which integrations/starlette_client reaches through\n.apps -- and it does NOT declare httpx in its metadata (cryptography, joserfc,\nnothing else). httpx used to arrive free as mcp 1.x's transitive. mcp 2 carries\nhttpx2, a SEPARATE package and not an alias, so nothing pulled httpx any more.\n0.2.265's CI was green because httpx was declared in the DEV GROUP, which is\nexactly what `uv sync --frozen --no-dev` leaves out when it builds the server\nimage. Every gate we own installs dev. The one environment that does not is\nproduction:\n\n  File \"/app/src/reveille/daemon.py\", line 5310, in _oidc_boot\n    from authlib.integrations.starlette_client import OAuth\n  ModuleNotFoundError: No module named 'httpx'\n\nhttpx is now a RUNTIME dependency, where authlib actually needs it.\n\nWHY THE fresh-resolve GATE SLEPT THROUGH IT, which is the part worth keeping.\nIt resolved from the SPEC rather than the lock -- correct, and the reason it\nexists -- and then ran `import reveille.daemon`. A MODULE IMPORT IS NOT A BOOT.\nmain() is never executed by an import, so every FUNCTION-LEVEL import main()\nreaches stays unevaluated, and `_oidc_boot()` opens with that authlib line\nunconditionally. The gate now drives the BOOT PATH: `_oidc_boot(env={})`, which\nconfigures no doors but performs the import, plus `import uvicorn`, main()'s\nother deferred import. Proven by putting httpx back in the dev group: the gate\nreproduces the production traceback instead of passing.\n\nUPGRADING authlib DOES NOT FIX IT, measured rather than assumed: 1.8.0, the\nlatest, still hardcodes the same import and still omits it from its metadata.\nNor can we move to httpx2 -- the name is hardcoded in a third party we do not\ncontrol, and dropping it means dropping the OIDC door. The two HTTP clients\ncoexist because mcp ships one and authlib needs the other; that is not a\nchoice this repo made and not one it can unmake.\n"),
    ("0.2.265",
     "0.2.265 THE DAEMON SPEAKS MCP 2 (operator direction: \"I want to make sure we\nare using the LATEST MCP tools\"; ruled 23639, renumbered 23656). A CLEAN\nCUTOVER, not a bound: `\"mcp>=2.2,<3\"`. The `<3` is not a legacy shim -- a MAJOR\narrives by RULING, never by resolve, which is the defect class this closes.\n\nWHAT WAS BROKEN. pyproject carried `\"mcp>=1.2\"` with no upper bound, so\n`uv tool install` and waked's converge -- neither of which reads uv.lock --\ntook mcp 2.2.0 the moment it published, and daemon.py died at import:\n`ModuleNotFoundError: No module named 'mcp.server.fastmcp'`. uv.lock pinned\n1.28.1, so the repo venv, CI and `make up` were all green and BLIND to it.\nExposure was the fresh native BROKER install; bodies were never at risk --\ndaemon.py is the only module in src/ that imports mcp, and a body reaches MCP\nover HTTP.\n\nSERVER. `from mcp.server.mcpserver import Context, MCPServer`;\ntransport_security keeps its path. v2 moved stateless_http, json_response and\ntransport_security OFF the constructor ONTO streamable_http_app(), so\n`MCPServer(\"reveille\")` is bare and build_app() carries all three -- and the\nsix-line comment explaining why DNS-rebinding validation is off MOVED WITH\nTHEM, because a reason parked away from its argument is a reason nobody\nreads. `mcp_app.router.lifespan_context` still works under v2 and is KEPT;\nthe guide's `session_manager.run()` form is not needed and was not adopted on\nspeculation. Context is annotation-only here, so v2's message=/data= rename\nand the client_id removal cost nothing.\n\nTHE WEBSOCKET PLANE IS UNTOUCHED, asserted because it is the question a\nreader will have: /wake and /feed are starlette WebSocketRoutes sitting BESIDE\nthe mcp Mount, waked imports websockets and never mcp, and the lifespan was\ndriven under 2.2.0 with both routes still mounted on the way out.\n\nCLIENTS, 11 sites. The symbol rename `streamablehttp_client` ->\n`streamable_http_client` ran through `uv run symbol-rename` (0.2.264) -- and\nWAS NOT ENOUGH, which is the part worth reading. v2's signature is\n`(url, *, http_client=None, terminate_on_close=True)`: `headers=` is GONE and\nit yields a 2-TUPLE where v1 yielded three. A symbol rename cannot see an\narity change, so the call shape was rewritten by a libcst codemod -- including\nthe sites that unpack a LOCAL HELPER wrapping the client (scripts/distill.py\n_client, tests/smoke_ws.py session), which a match on the client's own name\nwould have missed. Each site now builds its own AsyncClient with an EXPLICIT\ntimeout: a bare one carries httpx's 5 s flat deadline, which kills the\nlong-lived GET stream.\n\nTHE REFUSAL CONTRACT NEARLY DIED SILENTLY, and it is the find worth\nreading. mcp 2.x reports ONLY `ToolError` to the caller: anything else\nbecomes `UnexpectedToolError`, whose message is the bare string\n`Error executing tool <name>` with the original text deliberately\nwithheld. The daemon raises store.AccessError / AuthError / BusError /\nValueError and never ToolError, so on the wire EVERY refusal on the bus\ncollapsed to one indistinguishable sentence -- `fact is over 1000 chars`,\n`is it joined?`, the AuthError that names `reveille init` as the remedy,\nall of it gone, with is_error still true. An agent could tell THAT it was\nrefused and never WHY. Nothing in the suite would have noticed, because\nthe suite calls the tools IN PROCESS where the exception is unchanged;\nit took a live v2 client against a scratch broker to see it.\n\n`@tool()` is daemon.py's own decorator over `@mcp.tool()`: it registers a\nwrapper translating those four into ToolError, and RETURNS THE ORIGINAL\nFUNCTION, so the translation exists on the wire and nowhere else. An\nin-process caller still sees store.AuthError, which is what keeps the\nsuite from having to import mcp to assert on a refusal -- the same blast\nradius argument as everything else here. A crash stays a crash: anything\nunlisted is still UnexpectedToolError, logged with its traceback and\nwithheld, which is correct for a genuine bug.\n\nALSO SILENT, AND MEASURED TWICE: v2 renamed the RESULT field to `is_error`\nand the tool's schema field to `input_schema`. v1's `isError`\nRAISE AttributeError rather than answering, so a test carried over\nunchanged fails for the WRONG REASON and invites deleting the assertion\ninstead of reading it. Lesson an-in-process-suite-cannot-see-a-wire-translation.\n\nTHE WRAPPER DOES NOT MOVE THE SCHEMA, proven from the wire rather than from\ninspect: tools/list serves 23 tools, and memory_add still advertises\nfact/kind/scope/entities/source/supersedes/occurred with fact+kind required,\nno `ctx` and no `args`/`kwargs`. Had functools.wraps stopped reaching mcp,\nevery tool would advertise a two-argument schema and Context injection would\nstop resolving -- the entire tool surface broken at once, with the suite\nstill green because an in-process caller never reads a schema.\n\nAGENT IMAGE 0.2.41 -> 0.2.42. docker/agent-probe is a baked input and it\nspeaks the client API, so the tag moves with it (14469) across all three pin\nsites. Its `--with` spec is the SAME STRING as pyproject's, gated by equality\nrather than by two greps that each look right (6e493fe8).\n"),
    ("0.2.264",
    ...
    ("0.2.250",
     "0.2.250 THE RING NAMES THE FILE IT CAME FROM (PR #275; measured on a live\nnative agent 2026-09-15). I4 makes the SESSION the only thing that deletes a\nspool entry and the doctrine told it to rm the specific files it handled,\nnever a glob -- but the ring carried no filename, so a body had to list new/\nand match by eye. Nothing enforced the drain and nothing noticed a missed one.\n\nA missed one is not inert. --follow keeps `seen` in MEMORY, so the next\nwatcher PROCESS starts empty and re-prints every entry still in new/: under a\nharness that re-arms on a timeout (Claude Code's Monitor tool, 1800 s) an\nacked ring is re-delivered every cycle, forever. One already-answered message\n(id 20, acked 13:17 the previous day) replayed at 06:42, 07:16, 08:01, 08:35\nand 09:06, two model turns each, the agent correctly concluding \"stale\nreplay, nothing owed\" every time. The spool held exactly one file. Deleting\nit ended it.\n\nThe frame now carries `spool`: the path the watcher read it from, added by\nthe watcher because it is the only component that knows it. Additive -- the\ndaemon's own fields are untouched, and text that is not a JSON object is\nprinted exactly as it arrived, because I3 outranks the annotation and a ring\nthat cannot be annotated is still a ring. usage(), the CLAUDE.md block\ndaemon.py serves, the block `reveille init` writes and the Stop hook's\nverdict all name the key and say what a missed drain costs.\n\nKNOWN GAP, NOT CLOSED HERE: the path is absolute only because $REVEILLE_SPOOL\nnormally is. spool.base_dir() returns the override verbatim, so a relative\none emits a relative key and the doctrine's \"absolute path\" stops being true\n-- the watcher's cwd is not the session's. os.path.abspath in _emit closes\nit; the gate asserts isfile, which a relative path passes whenever cwd\nhappens to match.\n"),
    ("0.2.249",
     "0.2.249 THE RING LANDS AT KQUEUE SPEED (operator order, PR #265; INSTALL\n#264 named the gap). wake-watch waited on inotify where Linux offered it and\na 2s poll everywhere else, so every ring on a Mac ate up to two seconds.\nwatch.py now dispatches through one `_arm(path)`: inotify on Linux, kqueue\n(KQ_FILTER_VNODE, NOTE_WRITE|NOTE_EXTEND, EV_CLEAR) on the BSDs and macOS,\nthe 2s poll where neither answers. Both the one-shot and --follow call sites\nshare it; the duplicated select/sleep blocks are gone. Every path still\nre-scans the directory after the wait, so the backend is latency, never\ncorrectness. Gated: the Linux behavioural tests unchanged, the kqueue branch\nproven WIRED under a fake select (arm flags exact, control(None,4,30), fstat\nproves the dirfd closed). macOS field run unverified: no Mac in the fleet.\nConverge delivers it; no image bump."),
    ("0.2.248",
     "0.2.248 THE PROGRAM WRAPPED IT, NOT TMUX (operator 14869, ruled 14873; PR\n#262). Claude Code prints its /login oauth URL with REAL newlines at the\nterminal width (Ink), so `capture-pane -J` returns the pieces unjoined --\ntmux joins only what tmux wrapped -- and 0.2.242's ask-tmux answered a\ntruncated line while the continuation rows opened nothing. Measured on a\n214-col pane: two rows of exactly 214, `-J` giving back the same two.\nUpstream will not emit OSC 8 (claude-code #23804). Now `/read/pane` also\nreturns `cols` from a second frozen argv (`tmux display-message -p -t agent\n'#{pane_width}'`), and the page rebuilds the run of `-J` lines whose length\nis EXACTLY cols, both directions from the click -- equality with the pane\nwidth is the discipline: tmux-wrapped comes back longer, a line that ended\nnear the edge shorter, only a program's cut lands on it. No width, no\nchange. Accepted cost pinned: a URL ending exactly at the edge followed by a\nnon-blank line takes its first token. OPEN: with the browser NARROWER than\nthe pane, the continuation row still yields a cut address (the row-to-line\nmapping stops when the client cut the row before the line closed).\n"),
    ("0.2.247",
     "0.2.247 THE NAME WEARS ITS OWN COLOUR, THE REMAINDER (operator 14831, ruled\n14832/14835; PR #261). The `to:` button renders each bound target as a span\nin its agent's colour (toLabel keeps the plain string for title/aria); a\nposted body colours `@name` and `#room@name` only when the name is in the\ncurrent roster, over already-escaped text so it can never inject; unknown\nnames stay plain. THE GATE IS A COMPARISON, NOT A PIN: scripts/ui-drive\nreads the rail swatch's computed colour and asserts the panel row, the\nchip, the to: span and the posted mention EQUAL it, both viewports, no hsl\nliteral -- and its iPhone run found two real defects: #taPanel and #taChips\nlived inside .ctop, which the phone collapses, so the type-ahead and the\nbound chips did not exist on phones at all. Both moved out of .ctop; the\nanchor (#composer) is unchanged. Fixture: uilab.seed joins the first extra\nagent, because presence is members rows and a token alone never showed in\nthe rail.\n"),
    ("0.2.246",
     "0.2.246 THE ARM MUST NOT ASK THE RENDERER (operator field report 14844, ruled\n14845 amending 14682; PR #260). 0.2.242 made a click on a wrapped URL ask\ntmux for the whole line -- but the decision WHETHER to ask walked xterm's\n`isWrapped`, and tmux paints the client row by row, so a line tmux wrapped\narrives as separate xterm lines with no wrap flag: rows 2 and 3 of a\n450-char login URL were dead while row 1 worked, and tmux was never asked\nfor the URL it held. The arm predicate is now a pre-filter over the run of\nconsecutive non-empty rows around the click -- characters only, no flag,\nno width -- and it decides nothing about WHAT opens: tmux's `-J` text still\ndoes, and a false arm costs one blank tab that closes itself. paneXtermUrl\nmoved inside the pure fence so node drives it; the gate pins the\noperator's own three-row URL with isWrapped false on every row (red on the\nold walk) and proves the flag is not read either way. Field proof owed:\na click on the LAST row, the opened tab's address bar.\n"),
    ("0.2.245",
     "0.2.245 THE NAME WEARS ITS OWN COLOR (operator 14831, minutes after the\npopup's field proof closed). The type-ahead row now carries the agent's\nicon in the SAME color the feed gives that agent -- one hue function\n(hue/color/tint/initials) shared by the panel row, the feed avatar and\nthe bound chip, so one agent is one color everywhere on the page. The\nbound chip in the unified to: area color-coordinates the same way; rooms\nhash their own name through the same function. The textarea itself\ncannot color a substring (a plain textarea has no rich text), so the\nchip is where the bound name wears its color -- in-text coloring would\nmean a contenteditable or overlay rewrite and rides only if ruled worth\nits fragility. Pinned as code tokens on both render sites.\n"),
    ("0.2.244",
     "0.2.244 THE TERMINAL PAGE WAITS FOR ITS OWN BOOT (13407, ruled 13408/13410;\nPR #257, blocked 14792 then fixed 14798). The server half shipped in 0.2.2xx:\nfetch_with_boot_grace retries a refused attach to 15s and then answers 502\nnaming the state. The page half never did -- an iframe does not retry, so\nthat 502 sat in the frame until a human closed and reopened the tab, the\nmanual retry the ladder existed to replace. Now the tab opens on a waiting\npanel, holds the real url in `pending`, probes ttyd's own /token through the\nsame two gates (6 tries, 5s apart), and becomes the terminal when one\nanswers. Branches on STATUS only: 502 is asked again, 403/401 are answers\nand are rendered once. A tab closed while waiting releases its grant. And\nthe defect the source pins could not see: drawTabs reconciled frames by gid\nalone, so the waiting panel was found again after the url was promoted and\nthe iframe was never built -- the terminal never appeared on the success\npath. A frame is now keyed by (gid, KIND). Found and re-proven in a real\nbrowser: scripts/ui-drive gained two scenes (press play then connect; an\nanswer is not retried), red on the pre-fix page, green after. Field proof\nstill owed: play on a stopped agent, terminal arrives untouched.\n"),
    ("0.2.243",
     "0.2.243 CONVERGE UPGRADES INSIDE THE VENV -- NEVER UNLINK-FIRST (ruled\n14716, from the 12/12 rollback night; mechanism proven on a scratch goat,\n14714/14718). `uv tool install --force` strips every ~/.local/bin console\nscript before it builds: 108 measured seconds of no `reveille` on a cold\ncontainer, under the entrypoint's own init and every Stop hook -- so every\n0.2.36 body whose baked toolchain trailed the broker died `command not\nfound` at boot, never attached, and the launcher's presence gate correctly\nrolled all twelve back. The gate held; the reinstall form was the defect.\nConverge now runs `uv pip install --python <tool venv> --reinstall-package\nreveille <git>` -- the shims are symlinks into that venv and an in-venv\nreinstall never touches them (0/16 shim-missing polls vs 13/26 measured).\nThe cli's persist step drops --force (a durable copy that PATH cannot see\ngets its PATH fixed, not a reinstall over live daemons) and the panel's\ntaught command drops it too; the source gate bans the unlink-first form\nfrom src/ entirely. Gate stays 120s -- a report predicate would have\nshipped a deaf body. Boot-report `waked is running regardless` conviction\nand the rollback-reason surfacing ride later slices.\n"),
    ("0.2.242",
     "0.2.242 A WRAPPED URL IS ONE URL (operator field report; ruled 14670/14676/\n14679/14682, PR #254). A terminal pane and a browser client that disagree\nabout width -- tmux `window-size largest` lets them -- show the link regex a\nCUT line, so a click opened the address truncated at the client's edge (or,\nin the mirror case, with the pane border glued on). The page now ASKS tmux:\na new launcher read verb, `/agents/<name>/read/pane`, runs the frozen argv\n`tmux -u capture-pane -p -J` with one bounded integer, under the invariant\nthat an exec is a read IFF its argv is a compile-time constant and its output\nis information the caller already holds. The click is intercepted at window\ncapture on the iframe's own window (ttyd's handler never yields otherwise),\nthe gesture is spent synchronously on a blank tab, and the tab is navigated\nonce tmux answers; a spot tmux mapped with no URL closes the tab, a spot it\ncould not map falls back to xterm's own answer. No wrap heuristic anywhere.\nField proof still owed: the opened tab's address bar.\n"),
    ("0.2.241",
     "0.2.241 THE POPUP ANCHORS TO ITS OWN COMPOSER (field defect, operator's\nscreenshot 14652, diagnosed at 14653). The inline-addressing type-ahead\nshipped working and invisible: #taPanel is bottom-anchored absolute, its\nparent form carried no position, so the panel's containing block was\n#main -- the whole column -- and 'just above my container' meant just\nabove the top of the page. `@rev` opened the panel with matches in it,\noff-screen. One declaration: #composer{position:relative}, with the\ncontainment pinned in the page-source gate (CI has no browser; the pin\nis the regression fence, the operator's next `@rev` the field proof).\nThe working to: picker never had the bug because its wrapper was\npositioned -- same rule, different parent, different result.\n"),
    ("0.2.240",
     "0.2.240 THE TAIL STOPS GROWING (ruling 14631, at 156 entries): usage()'s\ndefault tail becomes a WINDOW -- the newest twelve entries keep their\ntitled one-liners and everything older collapses to one counted line\nnaming the oldest titled version, so usage(since=) reaches all of it and\na release costs the 24000 budget nothing, forever. The crossing that\nforced it was arithmetic: one ~37-wire-char title per release against a\nreference that never shrinks. 13213's guarantee holds unchanged over the\nwindow; beyond it the counted since= line is the existence guarantee.\n\nAlso folded (14640): the launcher's pin step resolves uv itself -- PATH,\nthen the two homes uv installs to, then a refusal naming all three --\nclosing the line the 0.2.239 deploy's first attempt died on under a\ndetached session with no login PATH. From this release, `bash -lc` is no\nlonger load-bearing for a deploy.\n"),
    ("0.2.239",
     "0.2.239 THE REFUSAL STAYS UNTIL READ, AND SAYS THE TRUE VERDICT (rulings\n14611/14614/14621, all from one evening of field friction). A red toast\nhas no timer -- it stays until clicked; a timer on a refusal races the\nreader. An Agents-panel row action that is refused writes its sentence\ninto the open tab's state pane; the toast is the alert, not the record.\nAdvice checks presence before advising: an identity ALIVE on another\nmachine gets 'nothing here needs fixing -- use the return ticket, never\nre-provision', because the old sentence walked the operator into the\nduplicate-mint 409; the upgrade route reads the hive half with the\nrequest's own cookie and degrades to the plain advice when presence is\nunreadable.\n\nInline addressing, amended: a name TYPED IN FULL binds when its token\ncloses (whitespace, punctuation, or submit) while the popup is open -- no\nTab needed; a paste still binds nothing, mechanically, because a paste\nnever opens the popup. And one 'to:' area: label and picker show the\nunion of picker set and inline bindings; inline entries wear their @ and\nleave only when their token leaves the text or their chip is removed.\n\nON THE RECORD, the lesson re-taught: the slice's first push rode a\n`pytest | tail` whose exit code was the tail's, and shipped three reds\nthe source-pin gates then caught in CI -- a piped test run cannot gate a\ncommit, second offence, same author.\n"),
    ("0.2.238",
     "0.2.238 THE TARGET IS WHAT YOU PICKED (ruling 14523). Inline @/#\naddressing in the web composer: '@'+1 char opens this room's members,\n'#'+1 char opens held rooms, '#Room@' opens THAT room's members with no\nextra character. A target exists ONLY where the type-ahead was ACCEPTED\n(Tab/Enter/click) -- the accept binds room and agent IDS (names are\nper-room, DES-011 s2) and shows a removable chip; typed or pasted text is\na literal even on an exact match, and matching stops at whitespace so a\npasted '#Some Comment' can never resolve past its first word. Submit\nconsumes only the picker set plus the accepted bindings through a pure\nbuilder that takes no body text; zero resolved targets stays a broadcast;\n'#Room' posts a NEW ROOT there (replies never cross rooms, reply_to is\nstripped from cross-room copies); duplicates collapse; every copy shares\none 14434 send_group. Permission stays the broker's refusal -- the menu\nis convenience. The tokenizer and builder are fenced TA-PURE in the\nserved page and gated verbatim under node.\n\nKNOWN EDGE, recorded so nobody rediscovers it: the member list for the\ntype-ahead caches per room for the page lifetime -- someone who joins\nafter the page loaded appears at the next reload; if it bites, the fix is\ninvalidating on the feed's join events, not a TTL. Also folded: the 14434\nfeed grouping drops its adjacency assumption -- chip lookup is always by\nrecorded group, so an interleaved sender can no longer split a set.\n"),
    ("0.2.237",
     "0.2.237 ONE SUBMIT, ONE FEED ROW (ruling 14434, from the operator's\nscreenshot of one multi-recipient send rendering as N duplicate entries).\nThe composer mints one id per submit and every copy carries it as\nmessages.send_group (schema v44, additive, NULL for all history -- no\nbackfill, because a group inferred after the fact asserts a set the store\ncannot support). The feed collapses copies sharing (room, sender, group)\ninto one row: body printed once, each recipient a chip carrying its own\nmessage id and opening its own thread. RECORDED AT SEND TIME, NEVER\nINFERRED AT RENDER -- two deliberate identical unicasts carry no group and\ncan never merge. Render-only for humans: agents receive N messages and ack\nN, unchanged; send_group rides every read surface as advisory data any\nclient may group by and none must. The web route bounds the client-minted\nid at 64 chars; the MCP send tool mints nothing -- an agent send has no\nset.\n"),
    ("0.2.236",
     "0.2.236 A DAEMON THAT IS ALIVE CAN STILL BE DEAF -- THE WEDGE RE-EXECS\nITSELF (ruling 14445, held and sharpened at 14472). Field case: waked\nretried opening handshakes for eleven minutes -- intermittently for five\ndays, 4545 log lines -- while a fresh client from the same host connected\nin 0.13s; the process was up, logging and deaf, the mode death-supervision\ncannot see. After WEDGE_REEXEC_N=10 consecutive sessions in which the\nbroker never SPOKE (a received frame is the reset -- registration and\nrefusal both speak; an opened socket alone proves nothing), waked re-execs\nitself on the same code via convergence's exec-in-place path: fresh client\nstate, same pid, same flock, its own marker line so waked.log's death\narithmetic stays readable (contract superseded to the three-term form).\nThe budget is five re-execs, carried in a spool-side file; spent, the\ndaemon stays on the retry ladder and writes BUS-DEAF where the\nbusdeaf-probe writes, and recovery erases only the healer's own\nhandwriting. ConnectionRefusedError never joins the streak: a prompt\nrefusal is a working socket layer with an absent broker, so a planned\n`make up` restart cannot end in a re-exec or a false deafness report.\n\nTHE DEFECT THE HELD REVIEW'S TEST CAUGHT, on the record: as first shipped,\nthe reset ran after the session RETURNED -- a healthy held session never\nreturns, so it never shed its budget and the streak carried across a\nworking connection. The reviewer demanded the one test proving the healer\nSTOPS (every other test proved it fires); that test failed the shape and\nthe reset moved into the frame loop, firing on the first spoken frame.\nThe 1011 keepalive root cause behind the field case remains open.\n"),
    ("0.2.235",
     "0.2.235 THE ARM STEP SURVIVES A SESSION BOUNDARY (ruling 13582, from the\n2026-08-21 deafness: 39 rings pooled while a body asserted 'already armed'\nfrom memory). The watcher-arm doctrine gains two sentences on every surface\na booting body reads -- a session boundary kills every watcher the old\nsession armed, and 'already armed' is a claim about a live process, verified\nby liveness, never by memory: at boot arm unconditionally. Sites: the\nreveille init managed block, USAGE's first-person reachability block (and\nwith it the repo CLAUDE.md, whose pin gate's CLOSE anchor moves to 'skipped\none costs every ring.' so the amendment sits inside the pinned span), and a\nqualifier on the second-person walkthrough's ONCE PER SESSION phrase -- the\nreference still fits the 24000-char usage() budget, which is why the second\ncopy is a qualifier and not a second statement.\n\nALSO THE DAY'S RECORD, CI: the gate's ffmpeg moved n8.1.2-44 -> n8.1.2-50\nbecause BtbN deleted the -44 autobuild (dated autobuilds live ~14 days); the\ntarball is mirrored onto this repo's ci-ffmpeg-* release and THE PIN IS NOW\nTHE SHA256, ASSERTED ON EVERY BUILD, rather than the URL. The actions cache\nand its executable guard are gone: a cache is a bypass around every check\ninside the miss branch -- the first 'green' assert had in fact never\nexecuted (run 33965054315, cache hit), and the proven gate is the one that\nwent red on a flipped hex char in 8 seconds with pytest never reached (run\n33965747004).\n"),
    ("0.2.234",
...
def tool():
    ...
    A REFUSAL CROSSES THE WIRE WITH ITS REASON; A CRASH CROSSES WITHOUT ONE.
    The translation set is CLOSED -- store.AccessError, store.AuthError,
    store.BusError, ValueError -> ToolError -- and everything else stays
    UnexpectedToolError deliberately. Widening it would start shipping the text
    of genuine bugs to callers; narrowing it makes a refusal unreadable again.
...
_DIGEST_FRAME = (
    ...
    "copied EXACTLY from the data: [kind:id8]. Never invent a tag, never alter one, never "
...
def digest_row_budget():
    ...
    The operator's ceiling is a TOKEN budget, and the fold's cost is per ROW,
    so one of them has to be converted -- and it is converted here, once, from
    a MEASURED cost per stored row rather than from a guess. At 50000 tokens
    and 116 tokens a row that is 431 rows.

    This is the term the digest never had. Nothing bounded what a fold was
    offered: a first run pulled every live row the agent could read, 1360 of
    them today, ~265000 tokens of source, and the note simply stopped wherever
    the batches ran out -- reported as 57% coverage, which measured the size of
    the store and not the quality of the fold."""
...
def _digest_prepare(conn, p, mentor_name=""):
    ...
    try:
        ...
        inputs = store.digest_inputs(conn, name=p.name, agent_id=p.agent_id,
                                     ...
                                     index_ceiling=DIGEST_MAX_TOKENS)
        with _digest_lock:
...
async def wake_ws(ws: WebSocket):
    ...
    try:
        ...
        #
        # DO NOT REMOVE THE BROADCAST FILTER -- it is `act["direct"]` now, and the
        # reason is unchanged. Ringing on a backlog broadcast would wake every agent
        # holding any unread broadcast on EVERY broker restart and every waked
        # respawn -- at a 15s reconnect backoff, a flapping broker rings the whole
        # fleet every 15 seconds. It also buys nothing: "hear it now" is a property
        # of the SEND path, and a backlog broadcast is by definition not now.
        #
        # COUNTED, NEVER HYDRATED (ruling 20404 B2). This asked store.inbox() and
        # then took two len()s of it -- the full _SEL join, the attachments and
        # scripts IN queries and two os.path.exists per unread row, for two
