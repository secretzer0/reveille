...
CHANGES_ENTRIES = (
    ("0.2.270",
     "0.2.270 A TAP IS NOT A HOLD (operator 23971, with a screenshot). A mouse\nCLICK on the talk button is pointerdown and pointerup about a hundred\nmilliseconds apart: the take is empty, its peak is zero, and the silence\nrefusal -- written for a browser with no real input device -- told the\noperator to check the OS input and the mic permission. Both were fine.\n\nThe verdict on a take is now decided on LENGTH before SIGNAL: under half a\nsecond was not held, and the toast says to press and HOLD (touch: tap to\nstart, tap again to stop). A held take that is still silent keeps the\ndevice message, because that one is true. talkVerdict() is pure and the\ngate runs it out of the served page.\n\nFOUND UNDERNEATH: pointerup can land while getUserMedia is still opening\nthe device, and talkStop found no recorder and returned -- so the mic\nstayed ON until the 60 s cap, in a tab that looked idle. The gesture keeps\nits own flag now and talkStart re-checks it after the await.\n"),
    ("0.2.269",
