...
CHANGES_ENTRIES = (
    ("0.2.230",
     '0.2.230 A REFUSAL STAYS WHERE THE USER WAS LOOKING (operator GO 13727; driven\nevidence 13709). Every SUCCESS state in these dialogs writes a line into its own\nstatus region -- "provisioning...", "LIVE: <name> is on the bus." -- and the one\nstate a user must ACT on WIPED that region and handed the sentence to a 5-second\ntoast: 470px from the dialog on desktop, ON TOP of the form on a phone, and gone\nbefore it was read. Form still filled, no trace of why nothing happened. Six\nhandlers -- agent edit, send-back, ask-a-host, move-it-here create, create,\nvisit-provision -- now route through one `failIn(el,msg)`: the words stay in the\nregion for the eye that comes back, the toast still fires for the ear that is\naway, and a retry clears the error colour before it re-runs.\n\nTHE FIRST GATE PROPOSED FOR THIS WAS AN INSTANCE OF THE DEFECT CLASS IT WAS\nGATING. A grep on one spelling of the clear-then-toast line matched THREE of the\nsix: it missed the edit path, whose clear and toast sit on separate lines, and\ntwo more that use different status variables -- so it would have gone GREEN over\nhalf the class while pointing at the sites named as proof. The gate that shipped\nis keyed on the PROPERTY -- a catch that clears any status element and hands the\nmessage to a timed toast -- and it ENUMERATES the class itself rather than\ntrusting a count, printing the six line numbers when it goes red. The driven\nscene adds the assertion that tests the DEFECT rather than the string: the\nsentence is STILL THERE at 6 seconds, after the toast has removed itself.\nAnything that only checked "present" passed before this slice and would pass\nafter a regression.\n\nTHE REFUSAL\'S OPENING CLAUSE WAS JARGON AND IS GONE. It began\n"claude_mode=home-login but ..." -- an internal config key naming a state the\nreader never set, standing in front of the only two sentences that tell them\nwhat to do. Found by opening the driven screenshot rather than by reading a\nsummary of it.\n\nDECLARED BEHAVIOUR CHANGE: EDIT FAILURES NOW BONK. The agent-edit path passed\ninfo=true to toast(), which suppressed the earcon -- an inconsistency rather\nthan a ruling, since contract 11577 says a refused toast bonks. All six failure\npaths now carry it.\n\nONE DOOR DRIVEN, SIX FIXED. The browser scene exercises the CREATE dialog only.\nThe other five are covered by the property test and by reading that\n`.pDim.err` applies to every one of the six status elements -- not by driving\nthem. No human has seen the other five refuse.\n'),
    ("0.2.229",
