...
## What this log became

The doc is the record; the rules live in `lessons()`, where the next body
reads them at boot. Two facts measured at body D's arrival are lessons of
their own -- neither is about the swap:

- `join-proves-arrival-never-reachability` -- zero waked, empty spool, and a
  clean join in the same instant. join() is a broker-side write; the daemon is
  a local process spawned by the Stop hook at the END of the turn.
- `a-new-body-arrives-on-the-images-toolchain-not-the-brokers` -- 0.2.177 in a
  container the broker answered 0.2.188 for. Converge runs at that same Stop
  boundary, so a first turn runs the image's code by construction.

The other two findings were already owned when this landed, and are cited
rather than re-filed:

- The state note lost to supersession (a new body joined between the push and
  the note) is ruling 12305/12320 R2, shipped 0.2.190: a credential superseded
  in the last five minutes keeps exactly two acts, `memory_add(kind='state')`
  and one send. That is why the doctrine's order is commit, THEN note, THEN
  verify -- the note is second because it is the only step nobody else can do.
- One directory holding two sessions that both ran the ritual on one ring is
  `one-directory-one-agent-but-ask-for-the-turn`.
