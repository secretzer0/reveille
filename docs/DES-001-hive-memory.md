...
### 16.7 A verb starts; rehydrate reads (architect 24173 on devops 24172, 2026-09-19)

The third live fold was healthy and the client still lost it: the held `POST /mcp` was closed at about 110 seconds while the fold ran on for its 85 minutes. **No verb holds a request open for a model** — the same family as 12750. `digest()` prepares on a worker thread (refusals, mentor, extraction: store-only, sub-second), spawns the fold on its own thread, and answers at once with `{started, batches, since, first_run}`; while one is in flight it answers `{started: false, why: "... (step k/K)"}`; after a failed attempt it answers the reason until `DIGEST_MIN_INTERVAL` passes. The verb and `POST /agent/digest` are one path. The result is read where it was always going to be read: `rehydrate()` page 1 row 1.
