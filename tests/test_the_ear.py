...
def test_the_page_has_one_mic_that_lands_words_in_the_box_and_never_sends():
    ...
    # EITHER mic gesture doubles as the audio unlock: talk and listen -- the
    # second also decodes the bell it is about to ring. (There were three while
    # the clip button existed; the operator had it stripped, 11831.)
    assert ear.count("try{vCtxUp();}catch(e){}") == 1 and \
        ear.count("try{vCtxUp();earconLoad();}catch(e){}") == 1, \
        "either mic gesture doubles as the audio unlock"
