...
def test_the_pair_on_disk_in_this_room_is_the_only_proof_and_one_clip_per_message(monkeypatch, tmp_path):
    conn, files, _ = _world(monkeypatch, tmp_path)
    (files / "x.webm").write_bytes(b"not a pair")
    store.record_file(conn, "x.webm", "r1", "travis")
    assert daemon._clip_of([{"url": "/files/x.webm", "clip": True}], "r1") is None, "a forged dict is nothing"
    for stem in ("a", "b"):
        (files / f"{stem}.webm").write_bytes(b"1"); (files / f"{stem}.m4a").write_bytes(b"1")
        store.record_file(conn, f"{stem}.webm", "r1", "travis")
        store.record_file(conn, f"{stem}.m4a", "r1", "travis")
    assert daemon._clip_of([{"url": "/files/a.webm"}], "r1") == "a"
    with pytest.raises(store.BusError, match="one clip per message"):
        daemon._clip_of([{"url": "/files/a.webm"}, {"url": "/files/b.webm"}], "r1")
    assert daemon._clip_of([{"url": "/files/../a.webm"}], "r1") is None
    # 11539: room A's pair named in a send to room B is an ordinary attachment, not B's voice.
    assert daemon._clip_of([{"url": "/files/a.webm"}], "r2") is None
    (files / "c.webm").write_bytes(b"1"); (files / "c.m4a").write_bytes(b"1")
    assert daemon._clip_of([{"url": "/files/c.webm"}], "r1") is None, "a pair no upload recorded is nobody's"
...
def test_the_upload_route_converts_a_wav_and_refuses_by_name_with_415(monkeypatch, tmp_path):
    """End to end through the route: the body streams in, the conversion runs
    off the event loop, the dict comes back converted; a refusal is a 415."""
    conn, files, _ = _world(monkeypatch, tmp_path)
    monkeypatch.setattr(daemon, "RAW_HOLD_S", 60.0)
    monkeypatch.setattr(daemon, "_principal", lambda request: _P())
    monkeypatch.setattr(daemon, "_act", lambda p: p)
    body = _wav(1.0)
    ...
    def req(name, data):
        ...
        return Request({"type": "http", "method": "POST", "path": "/upload", "headers": [
            (b"content-length", str(len(data)).encode()), (b"content-type", b"audio/wav")],
            "query_string": f"name={name}".encode(), "path_params": {}}, receive)

    async def go(name, data):
        r = await daemon.upload_http(req(name, data))
        return r.status_code, json.loads(r.body)
    code, att = asyncio.run(go("take.wav", body))
    assert code == 200 and att["clip"] is True and att["url"].endswith(".webm")
    daemon._raw_timers.pop(att["url"][len("/files/"):-5] + ".wav").cancel()
    monkeypatch.setattr(daemon, "AUDIO_ATTACH_MAX_S", 0.5)
    code, err = asyncio.run(go("long.wav", body))
    assert code == 415 and "the cap is 0 s" in err["error"] or "the cap is 1 s" in err["error"]
